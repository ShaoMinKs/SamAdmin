...
class FileService
{
    ...
    {
        return url('@admin/Uploadify/upload');
    }
    ...
    {
        $region = freshCache('storage_info.storage_qiniu_region');
        $isHttps = !!freshCache('storage_info.storage_qiniu_is_https');
        switch ($region) {
            case '华东':
                if ($isHttps) {
    ...
    public static function getUploadOssUrl()
    {
        ...
        return "{$protocol}://" . freshCache('storage_info.storage_oss_domain');
    }
    ...
    public static function getBaseUriQiniu()
    {
        ...
        switch (strtolower(freshCache('storage_info.storage_qiniu_is_https'))) {
            case 'https':
                return 'https://' . freshCache('storage_info.storage_qiniu_domain') . '/';
            case 'http':
                return 'http://' . freshCache('storage_info.storage_qiniu_domain') . '/';
            case 'auto':
                return '//' . freshCache('storage_info.storage_qiniu_domain') . '/';
            default:
    ...
    {
        switch (strtolower(freshCache('storage_info.storage_oss_is_https'))) {
            case 'https':
                return 'https://' . freshCache('storage_info.storage_oss_domain') . '/';
            case 'http':
                return 'http://' . freshCache('storage_info.storage_oss_domain') . '/';
            case 'auto':
                return '//' . freshCache('storage_info.storage_oss_domain') . '/';
            default:
    ...
    {
        switch (empty($storage) ? 'qiniu' : $storage) {
            case 'local':
                return file_exists(env('root_path') . 'static/upload/' . $filename);
            case 'qiniu':
                $auth = new Auth(freshCache('storage_info.storage_qiniu_access_key'), freshCache('storage_info.storage_qiniu_secret_key'));
                $bucketMgr = new BucketManager($auth);
                list($ret, $err) = $bucketMgr->stat('samadmin', $filename);
                return $err === null;
            case 'oss':
                $ossClient = new OssClient(freshCache('storage_info.storage_oss_keyid'), freshCache('storage_info.storage_oss_secret'), self::getBaseUriOss(), true);
    ...
    {
        switch (empty($storage) ? freshCache('storage_info.storage_type') : $storage) {
            case 'local':
                $file = env('root_path') . 'public/upload/' . $filename;
            ...
            case 'oss':
                $ossClient = new OssClient(freshCache('storage_info.storage_oss_keyid'), freshCache('storage_info.storage_oss_secret'), self::getBaseUriOss(), true);
                return $ossClient->getObject(freshCache('storage_info.storage_oss_bucket'), $filename);
            default:
    ...
    {
        $type = empty($file_storage) ? freshCache('storage_info.storage_type') : $file_storage;
    ...
    public static function local($filename, $content)
    {
        try {
            ...
            if (file_put_contents($realfile, $content)) {
                $url = pathinfo(request()->baseFile(true), PATHINFO_DIRNAME) . '/public/upload/' . $filename;
                return ['file' => $realfile, 'hash' => md5_file($realfile), 'key' => "public/upload/{$filename}", 'url' => $url];
            }
        } catch (Exception $err) {
    ...
    {
        $auth = new Auth(freshCache('storage_info.storage_qiniu_access_key'), freshCache('storage_info.storage_qiniu_secret_key'));
        $token = $auth->uploadToken(freshCache('storage_info.storage_qiniu_bucket'));
    ...
    {
        try {
            $endpoint = 'http://' . freshCache('storage_info.storage_oss_domain');
            $ossClient = new OssClient(freshCache('storage_info.storage_oss_keyid'), freshCache('storage_infostorage_oss_secret'), $endpoint, true);
            $result = $ossClient->putObject(freshCache('storage_info.storage_oss_bucket'), $filename, $content);
            $baseUrl = explode('://', $result['oss-request-url'])[1];
            if (strtolower(freshCache('storage_info.storage_oss_is_https')) === 'http') {
                $site_url = "http://{$baseUrl}";
            } elseif (strtolower(freshCache('storage_info.storage_oss_is_https')) === 'https') {
    ...
    public static function download($url, $isForce = false)
    {
        try {
            ...
            if (false === $isForce && ($siteUrl = self::getFileUrl($filename, 'local'))) {
                ...
                return ['file' => $realfile, 'hash' => md5_file($realfile), 'key' => "public/upload/{$filename}", 'url' => $siteUrl];
            }
