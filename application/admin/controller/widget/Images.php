...
use app\admin\model\SystemAttachmentCategory as Category;
...
class Images extends Base
{
   ...
   public function index()
   {
    ...
    //    }
       ...
       $this->assign(compact('typearray'));
        if($pid)  $where[] = ['pid','=',$pid];
             $where[]  = ['att_id','>',0];
        $list = SystemAttachmentModel::where($where)->paginate(10);
    ...
    public function upload()
    {
        ...
        $res = Upload::image('file','attach/'.date('Y').'/'.date('m').'/'.date('d'));
...
//            "state" => "SUCCESS"
    ...
    public function deleteimganddata($att_id){
        ...
        if($attinfo){
            @unlink(APP_PATH.ltrim($attinfo['att_dir'],'.'));
            @unlink(APP_PATH.ltrim($attinfo['satt_dir'],'.'));
