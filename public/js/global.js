...
function get_city(t,city,district,twon){
    ...
    var url = '/index.php/admin/Api/getRegion/level/2/parent_id/'+ parent_id;
    $.ajax({
        type : "get",
        url  : url,
        error: function(request) {
...
function get_area(t,district,twon){
    ...
    $('#'+twon_id).empty().css('display','none');
    var url = '/index.php/admin/Api/getRegion/level/3/parent_id/'+ parent_id;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
...
{	   	
	var upurl = '/index.php/Admin/Uploadify/upload/num/'+num+'/input/'+elementid+'/path/'+path+'/func/'+callback+'/fileType/'+fileType;
	var title = '上传图片';
    if(fileType == 'Flash'){
        title = '上传视频';
    }
